"""Tests for InterestDomainMatcher — checks if domain matches interest graph."""
import pytest
from unittest.mock import patch, MagicMock


def test_is_interest_domain_returns_true_for_exact_match():
    """When domain is in the graph, is_interest_domain returns True."""
    with patch("core.interest_domain_matcher.extract_interests", return_value=["github.com", "arxiv.org"]):
...
def test_is_interest_domain_returns_false_for_unknown():
    """When domain is not in graph, returns False."""
...
def test_get_interest_domains_returns_all():
    """Returns all domains from graph."""
    with patch("core.interest_domain_matcher.extract_interests", return_value=["github.com", "arxiv.org", "pytorch.org"]):
        from core.interest_domain_matcher import InterestDomainMatcher
        matcher = InterestDomainMatcher()
...
def test_get_interest_domains_excludes_non_domains():
    ...
    with patch("core.interest_domain_matcher.extract_interests", return_value=["github.com", "distributed systems", "RLHF"]):
        from core.interest_domain_matcher import InterestDomainMatcher
        matcher = InterestDomainMatcher()
        domains = matcher.get_interest_domains()
...
def test_empty_interests_returns_empty_set():
    ...
    with patch("core.interest_domain_matcher.extract_interests", return_value=[]):
        from core.interest_domain_matcher import InterestDomainMatcher
        matcher = InterestDomainMatcher()
...
def test_subdomain_matches_parent():
    ...
    with patch("core.interest_domain_matcher.extract_interests", return_value=["github.com"]):
        ...
        # raw.githubusercontent.com is NOT a subdomain of github.com (it's githubusercontent.com)


def test_infers_domain_from_topic_keyword():
    """Topic keywords like 'PyTorch' infer domain pytorch.org."""
    with patch("core.interest_domain_matcher.extract_interests", return_value=["PyTorch", "GitHub"]):
        from core.interest_domain_matcher import InterestDomainMatcher
        matcher = InterestDomainMatcher()
