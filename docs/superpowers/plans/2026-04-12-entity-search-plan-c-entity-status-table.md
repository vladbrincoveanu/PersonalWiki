# Entity Web Search — Plan C: Entity Status Table

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** After enrichment, perform web searches for entities that are tools/libraries/frameworks, then append a `## Entity Status` markdown table to the note (| Entity | Status | Source |).

**Architecture:** A new `vault/entity_status.py` module performs web searches for each tool/library entity and returns structured status dicts. `vault/writer.py` renders them as a markdown table. Entity type filter: `type == "library" or type == "framework" or type == "tool"` (case-insensitive).
...
| `vault/writer.py` | Add `## Entity Status` markdown table after Entities |
| `pipeline.py` | Thread `entity_status` through pipeline after enrichment, before write |
| `tests/test_entity_status.py` | New — tests for search, filtering, and table generation |
| `tests/test_writer.py` | Add test for ## Entity Status table |
...
from vault.entity_status import fetch_entity_status, _is_library_entity, _build_status_table
...
def test_build_status_table_generates_markdown_table():
    statuses = [
    ...
    table = _build_status_table(statuses)
    assert "| Entity | Status | Source |" in table
    assert "| PyTorch | v2.5.1 — actively maintained | PyPI |" in table
    assert "| crawl4ai | v0.3.0 — actively maintained | GitHub |" in table
...
def _build_status_table(statuses: list[dict]) -> str:
    """Build a markdown table from entity statuses."""
    if not statuses:
        return ""
    ...
    for s in statuses:
        version_str = f"v{s['version']}" if s.get("version") else ""
        status_str = f"{version_str} — {s['status']}" if version_str else s["status"]
        lines.append(f"| [[{s['name']}]] | {status_str} | {s.get('source', '')} |")
...
### Task 2: Update `vault/writer.py` to render `## Entity Status` table
...
```python
def test_write_note_entity_status_table():
    note = {
        "title": "Test Paper",
        "type": "paper",
        "tags": [],
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "## Entity Status" in post.content
        assert "| Entity | Status | Source |" in post.content
        assert "| [[PyTorch]] | v2.5.1 — actively maintained | PyPI |" in post.content


def test_write_note_no_entity_status_when_empty():
    note = {
        "title": "Test Paper",
        "type": "paper",
        "tags": [],
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        post = frontmatter.load(path)
        assert "## Entity Status" not in post.content
```

- [ ] **Step 2: Run tests to verify they fail**

```
pytest tests/test_writer.py::test_write_note_entity_status_table tests/test_writer.py::test_write_note_no_entity_status_when_empty -v
```
...
def write_note(
    ...
    from vault.entity_status import _build_status_table
    entity_status_section = ""
    if entity_statuses:
        table = _build_status_table(entity_statuses)
        if table:
            entity_status_section = f"\n## Entity Status\n{table}\n"
    ...
    body = (
        ...
        f"## Key Facts\n{facts_str}"
        f"{entities_section}{why_saved_section}{entity_status_section}{cross_links_section}{raw_section}"
    )
...
git commit -m "feat: add ## Entity Status table to notes"
```
...
- `## Entity Status` markdown table ✓
- Table columns: | Entity | Status | Source | ✓
...
- `_build_status_table(statuses: list[dict]) -> str` ✓
